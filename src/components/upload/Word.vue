    ...
    <div>
      <div class="flex modal_placeholder">
        <button
          ...
          type="button"
          class="btn bg-gray-100 dark:!btn-dark ltr:rounded-r-none rtl:rounded-l-none w-32 shadow-none">
    ...
    <vue-easy-lightbox
      ...
      <template #onerror>
        <div class="vel-img-wrapper">
          <iframe
            :src="`https://view.officeapps.live.com/op/embed.aspx?src=${outputUrl?.toString() ?? ''}`"
    ...
    setup(props, { emit }) {
      ...
      const onFileChanged = ($event: Event) => {
        ...
        if (target && target.files) {
          ...
          if (file.value.type !== 'application/msword' && file.value.type !== 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
            swalAlert('Tipe file .doc/.docx', 'error');
          }
